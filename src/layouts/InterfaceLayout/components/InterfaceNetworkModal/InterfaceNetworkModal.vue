...
export default {
  ...
  data() {
    return {
      types: networkTypes,
      selectedNetwork: networkTypes.WON,
      chainID: '',
      port: '',
      name: '',
      url: '',
  ...
  methods: {
    ...
    removeNetwork(net, idx) {
      ...
      if (net.service === this.$store.state.network.service) {
        if (this.customNetworks.length > 0) {
          ...
          this.switchNetwork(this.$store.state.Networks.won[0]);
        }
      }
    ...
    resetCompState() {
      this.selectedNetwork = { name: 'WON', name_long: 'WONCHAIN' };
      this.chainID = '';
      this.port = '';
      this.name = '';
      this.url = '';
