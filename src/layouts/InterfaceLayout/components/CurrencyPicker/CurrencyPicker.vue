...
export default {
  ...
  methods: {
    initLocalCurrency() {
      this.localCurrency = [{ name: 'WON', symbol: 'Ŵ' }];

      //this.$store.state.network.type.tokens.forEach(token =>
      //this.localCurrency.push({name:token.name,symbol:token.symbol}));
    },
    openDropdown() {
      this.open = !this.open;
    },
