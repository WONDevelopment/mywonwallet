...
SecalotEth.getErrorMessage = function(sw, operation) {
  ...
  if (sw === 0x6d00) {
    errorMessage =
      'Worldopennetwork wallet on your Secalot is not initialized.';
  } else if (sw === 0x6982) {
    if (operation === 'getPublicKey') {
      errorMessage =
        'Invalid PIN-code. Be careful, after entering a wrong PIN-code three times in a row, your Secalot Worldopennetwork wallet would be permanently wiped.';
    } else {
...
SecalotEth.prototype.signMessage = function(path, message, callback) {
  ...
  message =
    '\x19Worldopennetwork Signed Message:\n' +
