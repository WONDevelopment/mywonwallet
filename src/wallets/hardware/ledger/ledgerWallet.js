...
export default class LedgerWallet extends HardwareWalletInterface {
  ...
  // ============== (Start) Implementation of required WorldopennetworkJs-wallet interface methods =========
  getAddress() {
  ...
  // ============== (End) Implementation of required WorldopennetworkJs-wallet interface methods ===========
  ...
  async _getAccounts(_accountsLength, _accountsOffset) {
    ...
    try {
      ...
      const won = new Ledger(transport);
      const addresses = {};
      for (let i = accountsOffset; i < accountsOffset + accountsLength; i++) {
        const path =
          this.pathComponents.basePath +
          (this.pathComponents.index + i).toString();
        const address = await won.getAddress(path, this.askConfirm, false);
  ...
  async getAppConfig() {
    ...
    try {
      const won = new Ledger(transport);
      const appConfig = await won.getAppConfiguration();
      return appConfig;
    } catch (e) {
      throw e;
    } finally {
  ...
  async _signPersonalMessage(msgData) {
    ...
    try {
      ...
      const won = new Ledger(transport);
      const result = await won.signPersonalMessage(
        path,
  ...
  async _signTransaction(txData) {
    ...
    try {
      const won = new Ledger(transport);
      const tx = new WorldopennetworkTx(txData);
      ...
      const result = await won.signTransaction(
        path,
