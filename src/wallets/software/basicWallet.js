import * as utils from 'won-web3-utils';
import * as ethUtil from 'worldopennetworkjs-util';
import WorldopennetworkTx from 'ethereumjs-tx';
import WorldopennetworkWallet from 'ethereumjs-wallet';
...
export default class BasicWallet {
  ...
  // ============== (Start) WorldopennetworkJs-wallet interface methods ======================
  getPrivateKey() {
  ...
  // ============== (End) WorldopennetworkJs-wallet interface methods ======================
  ...
  _signTransaction(txData) {
    return new Promise((resolve, reject) => {
      try {
        ...
        txData.data = txData.data === '' ? '0x' : txData.data;
        const eTx = new WorldopennetworkTx(txData);
  ...
  static generateWallet() {
    ...
    const tempWallet = new WorldopennetworkWallet.generate();
  ...
  static createWallet(priv, pub, path, hwType, hwTransport) {
    ...
    if (typeof priv !== 'undefined') {
      ...
      wallet = WorldopennetworkWallet.fromPrivateKey(privateKey);
    }
  ...
  getWalletFromPrivKeyFile(strjson, password) {
    ...
    if (jsonArr.encseed != null)
      return new WorldopennetworkWallet.fromEthSale(strjson, password);
    // eslint-disable-next-line new-cap
    else if (jsonArr.Crypto != null || jsonArr.crypto != null)
      return new WorldopennetworkWallet.fromV3(strjson, password, true);
